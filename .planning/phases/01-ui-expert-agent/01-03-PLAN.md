...
must_haves:
  truths:
    - "Design data'dan Streamlit kodu uretilebilir"
    - "Uretilen kod syntax olarak gecerli (ast.parse)"
    - "Verification tool hallucination'lari yakaliyor"
  artifacts:
    - path: "src/sade_agents/skills/codegen_skills.py"
      provides: "Code generation ve verification tools"
      exports: ["generate_streamlit_code", "verify_generated_code", "load_reference_examples"]
      min_lines: 150
  key_links:
    - from: "src/sade_agents/skills/codegen_skills.py"
      to: "src/sade_agents/web/components/reference/"
      via: "reference library loading"
      pattern: "reference.*import|load_reference"
    - from: "src/sade_agents/skills/codegen_skills.py"
      to: "ast module"
      via: "syntax verification"
      pattern: "ast\\.parse"
---

<objective>
Code Generation Skills olusturmak - Streamlit kod uretimi ve verification.

Purpose: UI Expert Agent'in reference library'den ogrenip, dogru Streamlit kodu uretmesini saglamak.
Verification mekanizmasi hallucination'lari (olmayan API'ler) yakalamak icin kritik.

Output: 3 adet CrewAI tool (generate_streamlit_code, verify_generated_code, load_reference_examples)
</objective>
...
# Reference library (Plan 01'den)
@src/sade_agents/web/components/reference/__init__.py

# Design skills (Plan 02'den)
...
<task type="auto">
  <name>Task 1: Code Generation Skills</name>
  <files>src/sade_agents/skills/codegen_skills.py</files>
  <action>
Streamlit kod uretimi ve verification icin skills olustur:
...
```
  ...
  <verify>python -c "from src.sade_agents.skills.codegen_skills import generate_streamlit_code, verify_generated_code, load_reference_examples; print('OK')"</verify>
  <done>3 codegen skill import edilebilir, verification hallucination'lari yakaliyor</done>
</task>

<task type="auto">
  <name>Task 2: Skills Module Update for Codegen</name>
  <files>src/sade_agents/skills/__init__.py</files>
  <action>
Skills __init__.py dosyasina codegen_skills export'larini ekle:

Mevcut dosyayi oku, sonra codegen_skills import'larini ekle:
...
```
  ...
  <verify>python -c "from src.sade_agents.skills import generate_streamlit_code, verify_generated_code; print('OK')"</verify>
  <done>codegen_skills, skills modulu uzerinden export ediliyor</done>
</task>
...
# Verification test
python -c "
from src.sade_agents.skills.codegen_skills import verify_generated_code
import json
...
''')
data = json.loads(result)
print(f'Valid code test: {data[\"valid\"]}')

# Invalid code (hallucination)
result2 = verify_generated_code('''
def foo():
    st.card()  # Bu API yok!
''')
data2 = json.loads(result2)
print(f'Hallucination detected: {not data2[\"valid\"]}')
"
```
</verification>

<success_criteria>
- codegen_skills.py dosyasi mevcut ve import edilebilir
- generate_streamlit_code design data'dan kod uretiyor
- verify_generated_code syntax + API + type hint kontrolu yapiyor
- load_reference_examples reference library'den ornek yukluyor
- Hallucination (olmayan API) tespit ediliyor
- skills/__init__.py yeni export'lari icerir
</success_criteria>

<output>
After completion, create `.planning/phases/01-ui-expert-agent/01-03-SUMMARY.md`
</output>
